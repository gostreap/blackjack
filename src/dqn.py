...
class DQNSolver(nn.Module):
    ...
    def __init__(self, input_shape, n_actions):
        ...
        self.fc = nn.Sequential(nn.Linear(input_shape, 30), nn.ReLU(), nn.Linear(30, n_actions))
...
class DQNAgent:
    ...
    def test(self, n_episode):
        ...
        assert win + draw + loss == total_hand, "Error in win, draw, loss count : {} + {} + {} = {} != {}".format(
            win, draw, loss, win + draw + loss, total_hand
        )
...
if __name__ == "__main__":
    ...
    for env in [BlackjackEnv(), BlackjackDoubleDownEnv(), BlackjackDoubleDownSplitEnv()]:
        print("##### {} #####".format(env))
        dqn = DQNAgent(env, max_memory_size=30000, batch_size=128, gamma=0.2, lr=0.0025, epsilon=0.01)
        dqn.train(100000)
        dqn.test(20000)
